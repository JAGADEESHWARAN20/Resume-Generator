...
def DashboardFrame():
    ...
    dashboard_frame = ttk.LabelFrame(mainframe, text='DashBoard', width=700, height=800)
    dashboard_frame.pack(side='top', fill='both', expand=True)

    # Create a canvas to display the PDF
    canvas = tk.Canvas(dashboard_frame, bg='white', width=700, height=800)
    ...
    # Set the canvas scroll region to fit the PDF
        ...
        def projectframe2():
            ...
            if updater_frame is not None:
                ...
                def insertData():
                    ...
                    project_name = project_name_entry2.get()
                    explain_project = explain_project_entry2.get()
                    ...
                    c = conn.cursor()
                    try:
                        print(rrn_value,project_name,explain_project)
                        c.execute('''UPDATE Resume
                                    SET project_name_two = ?, 
                                        explain_two_project = ?
                                    WHERE rrn_value = ?''',
                                    (project_name, explain_project, rrn_value))
                        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
